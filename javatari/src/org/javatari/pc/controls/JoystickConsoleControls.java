...
public final class JoystickConsoleControls implements ConsoleControls {
    ...
    JoystickConsoleControls(AWTConsoleControls awtControls) {
        super();
    ...
    public synchronized void start() {
        ...
        try {
            ...
            joystick0DeviceOptions = joystick1DeviceOptions = new Vector<>();
    ...
    private void setupDeviceOptions(int joy0Device, int joy1Device) {
        ...
        joystick1DeviceOptions = new Vector<>();
        ...
        if (joystick0DeviceOption == null)
            joystick0DeviceOption = joystick0DeviceOptions.get(0);    // Use Auto if no option possible
        joystick1DeviceOption = null;
        for (DeviceOption opt : joystick1DeviceOptions)
            if (joy1Device == opt.device) joystick1DeviceOption = opt;
        if (joystick1DeviceOption == null)
            joystick1DeviceOption = joystick1DeviceOptions.get(0);    // Use Auto if no option possible
    ...
    private synchronized void update() {
        ...
        if (joystick0 != null) {
            ...
            if (ok) update(joystick0, joy0State, joy0Prefs, !swappedMode);
                ...
                joystick0 = null;
                videoMonitor.showOSD("P1" + " Joystick disconnected", true);
            }
        }
        if (joystick1 != null) {
            ok = joystick1.update();
            if (ok) update(joystick1, joy1State, joy1Prefs, swappedMode);
                ...
                joystick1 = null;
                videoMonitor.showOSD("P2" + " Joystick disconnected", true);
            }
        }
    ...
    public interface JoystickButtonDetectionListener {
        void joystickButtonDetected(int joystick, int button);
    }
    ...
    public static class DeviceOption {
        ...
        DeviceOption(int device, Info info) {
            this.device = device;
            this.info = info;
        }
    ...
    private static class Preferences {
        int device, xAxis, xAxisSignal, yAxis, yAxisSignal, paddleAxis, paddleAxisSignal, button, button2;
        int select, reset, pause, fastSpeed, deadzone, paddleCenter, paddleSens;
    }
