...
public final class AWTConsoleControls extends KeyFilteredRepeatsAdapter implements ConsoleControls, CartridgeInsertionListener {
    ...
    static final int KEY_SELECT = KeyEvent.VK_F11;
    static final int KEY_RESET = KeyEvent.VK_F12;
    static final int KEY_FAST_SPEED = KeyEvent.VK_TAB;
    ...
    public void connect(ConsoleControlsSocket controlsSocket, CartridgeSocket cartridgeSocket) {
        ...
        if (cartridgeSocket != null) cartridgeSocket.addInsertionListener(this);
    ...
    void processKeyEvent(int keyCode, int modifiers, boolean press) {
    ...
    private void addInputComponents(List<Component> inputs) {
        ...
        for (Component component : inputs) {
            ...
            for (KeyListener currListener : currListeners)
                if (currListener == this) continue Inputs;        // Avoid adding the same listener again
    ...
    private boolean checkLocalControlKey(int keyCode, int modif, boolean press) {
        if (press) {
            ...
            if (paddleMode) {
                ...
                switch (control) {
                    case JOY0_LEFT:
                        paddle0MovingLeft = true;
                        return true;
                    case JOY0_RIGHT:
                        paddle0MovingRight = true;
                        return true;
                    case JOY0_UP:
                    ...
                    case JOY0_DOWN:
                        ...
                        return true;
                    case JOY1_LEFT:
                        paddle1MovingLeft = true;
                        return true;
                    case JOY1_RIGHT:
                        paddle1MovingRight = true;
                        return true;
                    case JOY1_UP:
            ...
            if (paddleMode) {
                ...
                switch (control) {
                    case JOY0_LEFT:
                        paddle0MovingLeft = false;
                        return true;
                    case JOY0_RIGHT:
                        paddle0MovingRight = false;
                        return true;
                    case JOY1_LEFT:
                        paddle1MovingLeft = false;
                        return true;
                    case JOY1_RIGHT:
                        paddle1MovingRight = false;
                        return true;
                }
            }
        }
    ...
    private Control translatePaddleModeButtons(Control control) {
        switch (control) {
            case JOY0_BUTTON:
                return Control.PADDLE0_BUTTON;
            case JOY1_BUTTON:
                return Control.PADDLE1_BUTTON;
        }
        return control;
    }
    ...
    private void paddlesUpdateInactive() {
        ...
        if (paddlePositionUpdater.isAlive())
            try {
                paddlePositionUpdater.join(1000);
