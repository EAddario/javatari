...
public final class DesktopConsolePanel extends SlickFrame {
    ...
    DesktopConsolePanel(DesktopScreenWindow masterWindow, Monitor monitor) {
        super(false);
        docked = true;
    ...
    private void buildGUI() {
        ...
        setIconImages(Arrays.asList(masterWindow.icon64, masterWindow.icon32, masterWindow.favicon));
        addGlassPane();
        setSize(desiredSize());
        setResizable(false);
        ...
        contentHotspotPanel.addHotspot(
                ...
                () -> {
                    if (!docked) {
                        setVisible(false);
                        userClosed = true;
                        SwingHelper.edtInvokeLater(this::toggleRetract);
                    }
                });
    }
        ...
        sizeAdjustThread = new Thread("ConsolePanel Size Adjust") {
            ...
            public void run() {
                for (; delta[0] >= 0; delta[0] -= 8) {
                    try {
                        SwingHelper.edtSmartInvokeAndWait(() -> setSize(targetSize.width, targetSize.height + delta[0] * dir));
                        Thread.sleep(8);
                    } catch (InterruptedException ignored) {
                    }
                }
                SwingHelper.edtInvokeLater(() -> setSize(targetSize));
            }
        };
