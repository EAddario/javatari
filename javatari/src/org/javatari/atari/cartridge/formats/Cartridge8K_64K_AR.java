...
public final class Cartridge8K_64K_AR extends CartridgeBankedByBusMonitoring {
    ...
    public static final CartridgeFormat FORMAT = new CartridgeFormat("AR", "8K-64K Arcadia/Starpath/Supercharger") {
        ...
        public CartridgeFormatOption getOption(ROM rom) {
            ...
            if (rom.content.length % PART_SIZE != 0 || rom.content.length / PART_SIZE < 1 || rom.content.length / PART_SIZE > 8)
                return null;
    ...
    private void setControlRegister(byte controlRegister) {
        ...
        switch (banksConfig) {
            case 0:
                bank0AddressOffset = 2 * BANK_SIZE;
                bank1AddressOffset = BIOS_BANK_OFFSET;
                break;
            case 1:
                bank0AddressOffset = 0;
                bank1AddressOffset = BIOS_BANK_OFFSET;
                break;
            case 2:
                bank0AddressOffset = 2 * BANK_SIZE;
                bank1AddressOffset = 0;
                break;    // as used in Commie Mutants and many others
            case 3:
                bank0AddressOffset = 0;
                bank1AddressOffset = 2 * BANK_SIZE;
                break;    // as used in Suicide Mission
            case 4:
                bank0AddressOffset = 2 * BANK_SIZE;
                bank1AddressOffset = BIOS_BANK_OFFSET;
                break;
            case 5:
                bank0AddressOffset = BANK_SIZE;
                bank1AddressOffset = BIOS_BANK_OFFSET;
                break;
            case 6:
                bank0AddressOffset = 2 * BANK_SIZE;
                bank1AddressOffset = BANK_SIZE;
                break;    // as used in Killer Satellites
            case 7:
                bank0AddressOffset = BANK_SIZE;
                bank1AddressOffset = 2 * BANK_SIZE;
                break;    // as we use for 2k/4k ROM cloning		}
            default:
        ...
        //boolean biosRomPower = (controlRegister & 0x01) == 0;
    ...
    private void loadPart(int part) {
        ...
        do {
            ...
            if (tapeOffset > rom.content.length - 1) {
                ...
                if (tapeRewound) {
                    if (part == 0)
                        bus.tia.videoOutput().showOSD("Could not load Tape from Start. Not a Start Tape ROM!", true);
    ...
    private void loadHeaderData() {
        ...
        byte romPageCount = header[3];
        //byte romChecksum = header[4];
        romMultiLoadIndex = header[5];
        //int romProgressBarSpeed = (header[7] << 8) | (header[6] & 0xff);
    ...
    private void loadBIOSFromFile() {
        try {
            try (InputStream stream = getClass().getResourceAsStream("StarpathMockBios.bin")) {
                int totalRead = 0;
                do {
                    int read;
