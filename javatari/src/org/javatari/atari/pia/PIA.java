...
public final class PIA implements BUS16Bits, ClockDriven, ConsoleControlsInput {
    ...
    public void writeByte(int address, byte b) {
        ...
        if (reg == 0x04) {
            TIM1T = i;
            setTimerInterval(i, 1);
            return;
        }
        if (reg == 0x05) {
            TIM8T = i;
            setTimerInterval(i, 8);
            return;
        }
        if (reg == 0x06) {
            TIM64T = i;
            setTimerInterval(i, 64);
            return;
        }
        if (reg == 0x07) {
            T1024T = i;
            setTimerInterval(i, 1024);
            return;
        }
        if (reg == 0x02) {
            swchbWrite(i);
        ...
        if (reg == 0x03) {
            SWBCNT = i;
            debugInfo(String.format(">>>> Ineffective Write to PIA SWBCNT: %02x", i));
            return;
        }
        if (reg == 0x00) { /* SWCHA  = i; */
            debugInfo(String.format(">>>> Unsupported Write to PIA SWCHA: %02x", i));
            return;
        }    // Output to controllers not supported
        if (reg == 0x01) { /* SWACNT = i; */
            debugInfo(String.format(">>>> Unsupported Write to PIA SWACNT: %02x", i));
        }    // SWACNT configuration not supported
    ...
    public void controlStateChanged(ConsoleControls.Control control, boolean state) {
        ...
        switch (control) {
            case BLACK_WHITE:
                if ((SWCHB & 0x08) == 0) SWCHB |= 0x08;
                else SWCHB &= 0xf7;        //	0 = B/W, 1 = Color
                bus.tia.videoOutput().showOSD((SWCHB & 0x08) != 0 ? "COLOR" : "B/W", true);
                return;
            case DIFFICULTY0:
                if ((SWCHB & 0x40) == 0) SWCHB |= 0x40;
                else SWCHB &= 0xbf;        //  0 = Beginner, 1 = Advanced
                bus.tia.videoOutput().showOSD((SWCHB & 0x40) != 0 ? "P1 Expert" : "P1 Novice", true);
                return;
            case DIFFICULTY1:
                if ((SWCHB & 0x80) == 0) SWCHB |= 0x80;
                else SWCHB &= 0x7f;        //  0 = Beginner, 1 = Advanced
                bus.tia.videoOutput().showOSD((SWCHB & 0x80) != 0 ? "P2 Expert" : "P2 Novice", true);
        }
    }
