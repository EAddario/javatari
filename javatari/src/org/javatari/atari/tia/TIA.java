...
//	private int ENABL;		// ......1.  graphics (enable) ball
    ...
    public void clockPulse() {
        ...
        while (frames > 0 && powerOn) {
            ...
            if (paddle0Position >= 0 && !paddleCapacitorsGrounded)
                paddlesChargeCapacitors();    // Only if paddles are connected (position >= 0)
            finishLine();
    ...
    private void setPixelValue() {
        ...
        if (!playfieldPriority) {
            ...
            if (playfieldCurrentPixel) {
                FL = true;
                if (color > 0)
                    color = !playfieldScoreMode ? playfieldColor : (clock < 148 ? player0Color : player1Color);
            }
        }
    ...
    private void player0ClockCounter() {
        ...
        if (player0Counter == 156) {
            if (player0RecentReset) player0RecentReset = false;
                ...
                player0ScanCounter = 31 + player0ScanSpeed * (player0ScanSpeed == 4 ? 5 : 6);    // If Double or Quadruple size, delays 1 additional pixel
    ...
    private void player1ClockCounter() {
        ...
        if (player1Counter == 156) {
            if (player1RecentReset) player1RecentReset = false;
                ...
                player1ScanCounter = 31 + player1ScanSpeed * (player1ScanSpeed == 4 ? 5 : 6);    // If Double or Quadruple size, delays 1 additional pixel
    ...
    private void player0SetShape(int shape) {
        ...
        if (missile0ScanSpeed != speed) {
            ...
            if (missile0ScanCounter > 7)
                missile0ScanCounter = 7 + (missile0ScanCounter - 7) / missile0ScanSpeed * speed;
    ...
    private void player1SetShape(int shape) {
        ...
        if (missile1ScanSpeed != speed) {
            // if a copy is about to start, adjust for the new speed
            if (missile1ScanCounter > 7)
                missile1ScanCounter = 7 + (missile1ScanCounter - 7) / missile1ScanSpeed * speed;
                // if a copy is being scanned, kill the scan
...
//	private int CXCLR;		// <strobe>  clear collision latches
    ...
    private void vBlankSet(int blank) {
        ...
        if ((blank & 0x40) != 0) {
            ...
            if (controlsJOY0ButtonPressed) INPT4 &= 0x7f;
            else INPT4 |= 0x80;
            if (controlsJOY1ButtonPressed) INPT5 &= 0x7f;
            else INPT5 |= 0x80;
        }
    ...
    public void writeByte(int address, byte b) {
        ...
        if (reg == 0x1B) { /*GRP0   = i;*/
            playerDelaySpriteChange(0, i);
            return;
        }
        if (reg == 0x1C) { /*GRP1   = i;*/
            playerDelaySpriteChange(1, i);
            return;
        }
        if (reg == 0x02) { /*WSYNC  = i;*/
            bus.cpu.RDY = false;
            if (debug) debugPixel(DEBUG_WSYNC_COLOR);
            return;
        }    // <STROBE> Halts the CPU until the next HBLANK
        if (reg == 0x2A) { /*HMOVE  = i;*/
            hitHMOVE();
            return;
        }
        if (reg == 0x0D) {
            if (PF0 != i || playfieldDelayedChangePart == 0) playfieldDelaySpriteChange(0, i);
            return;
        }
        if (reg == 0x0E) {
            if (PF1 != i || playfieldDelayedChangePart == 1) playfieldDelaySpriteChange(1, i);
            return;
        }
        if (reg == 0x0F) {
            if (PF2 != i || playfieldDelayedChangePart == 2) playfieldDelaySpriteChange(2, i);
            return;
        }
        if (reg == 0x06) { /*COLUP0 = i;*/
            observableChange();
            if (!debug) player0Color = missile0Color = palette[i];
            return;
        }
        if (reg == 0x07) { /*COLUP1 = i;*/
            observableChange();
            if (!debug) player1Color = missile1Color = palette[i];
            return;
        }
        if (reg == 0x08) { /*COLUPF = i;*/
            observableChange();
            if (!debug) playfieldColor = ballColor = palette[i];
            return;
        }
        if (reg == 0x09) { /*COLUBK = i;*/
            observableChange();
            if (!debug) playfieldBackground = palette[i];
            return;
        }
        if (reg == 0x1D) { /*ENAM0  = i;*/
            observableChange();
            missile0Enabled = (i & 0x02) != 0;
            return;
        }
        if (reg == 0x1E) { /*ENAM1  = i;*/
            observableChange();
            missile1Enabled = (i & 0x02) != 0;
            return;
        }
        if (reg == 0x14) { /*RESBL  = i;*/
            hitRESBL();
        ...
        if (reg == 0x04) { /*NUSIZ0 = i;*/
            player0SetShape(i);
            return;
        }
        if (reg == 0x05) { /*NUSIZ1 = i;*/
            player1SetShape(i);
            return;
        }
        if (reg == 0x0A) { /*CTRLPF = i;*/
            playfieldAndBallSetShape(i);
            return;
        }
        if (reg == 0x0B) { /*REFP0  = i;*/
            observableChange();
            player0Reflected = (i & 0x08) != 0;
            return;
        }
        if (reg == 0x0C) { /*REFP1  = i;*/
            observableChange();
            player1Reflected = (i & 0x08) != 0;
            return;
        }
        if (reg == 0x25) { /*VDELP0 = i;*/
            observableChange();
            player0VerticalDelay = (i & 0x01) != 0;
            return;
        }
        if (reg == 0x26) { /*VDELP1 = i;*/
            observableChange();
            player1VerticalDelay = (i & 0x01) != 0;
            return;
        }
        if (reg == 0x27) { /*VDELBL = i;*/
            observableChange();
            ballVerticalDelay = (i & 0x01) != 0;
            return;
        }
        if (reg == 0x15) {
            AUDC0 = i;
            audioOutput.channel0().setControl(i & 0x0f);
            return;
        }
        if (reg == 0x16) {
            AUDC1 = i;
            audioOutput.channel1().setControl(i & 0x0f);
            return;
        }
        if (reg == 0x17) {
            AUDF0 = i;
            audioOutput.channel0().setDivider((i & 0x1f) + 1);
            return;
        }        // Bits 0-4, Divider from 1 to 32 )
        if (reg == 0x18) {
            AUDF1 = i;
            audioOutput.channel1().setDivider((i & 0x1f) + 1);
            return;
        }        // Bits 0-4, Divider from 1 to 32 )
        if (reg == 0x19) {
            AUDV0 = i;
            audioOutput.channel0().setVolume(i & 0x0f);
            return;
        }            // Bits 0-3, Volume from 0 to 15 )
        if (reg == 0x1A) {
            AUDV1 = i;
            audioOutput.channel1().setVolume(i & 0x0f);
            return;
        }            // Bits 0-3, Volume from 0 to 15 )
        if (reg == 0x28) { /*RESMP0 = i;*/
            missile0SetResetToPlayer(i);
            return;
        }
        if (reg == 0x29) { /*RESMP1 = i;*/
            missile1SetResetToPlayer(i);
            return;
        }
        if (reg == 0x01) { /*VBLANK = i;*/
            vBlankSet(i);
            return;
        }
        if (reg == 0x00) { /*VSYNC  = i;*/
            observableChange();
            vSyncOn = (i & 0x02) != 0;
            if (debug) debugPixel(VSYNC_COLOR);
            return;
        }
        if (reg == 0x2C) { /*CXCLR  = i;*/
            observableChange();
    ...
    public void loadState(TIAState state) {
        ...
        PF2 = state.PF2;
        AUDC0 = state.AUDC0;
        audioOutput.channel0().setControl(AUDC0 & 0x0f);        // Also update the Audio Generator
        AUDC1 = state.AUDC1;
