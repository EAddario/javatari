...
import org.javatari.general.m6502.instructions.*;
...
public final class M6502 implements ClockDriven {
    ...
    private int memoryReadWordWrappingPage(int address) {    // Accounts for the page-cross problem  (should wrap page)
    ...
    public String printMemory(int fromAddress, int count) {
        ...
        for (int i = 0; i < count; i++)
            str.append(String.format("%02x ", bus.readByte(fromAddress + i)));
        return str.toString();
    }
    ...
    private void showDebug(String title) {
    ...
    public M6502State saveState() {
        ...
        state.INTERRUPT_DISABLE = INTERRUPT_DISABLE;
        state.RDY = RDY;
        state.trace = trace;
        state.debug = debug;
    ...
    public void loadState(M6502State state) {
        ...
        DECIMAL_MODE = state.DECIMAL_MODE;
        INTERRUPT_DISABLE = state.INTERRUPT_DISABLE;
        RDY = state.RDY;
        trace = state.trace;
        debug = state.debug;
