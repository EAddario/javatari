...
public final class Parameters {
    ...
    private static final int DEFAULT_KEY_P0_DOWN = KeyEvent.VK_DOWN;
    private static final int DEFAULT_KEY_P0_BUTTON = KeyEvent.VK_SPACE;
    private static final int DEFAULT_KEY_P0_BUTTON2 = KeyEvent.VK_DELETE;
    private static final int DEFAULT_KEY_P1_LEFT = KeyEvent.VK_F;
    private static final int DEFAULT_KEY_P1_UP = KeyEvent.VK_T;
    private static final int DEFAULT_KEY_P1_RIGHT = KeyEvent.VK_H;
    ...
    private static final int DEFAULT_JOY_AXIS_SIGNAL = 1;
    private static final int DEFAULT_JOY_BUTTON = 0;
    private static final int DEFAULT_JOY_BUTTON2 = 1;
    private static final int DEFAULT_JOY_SELECT = 6;
    private static final int DEFAULT_JOY_RESET = 7;
    private static final int DEFAULT_JOY_FAST_SPPED = 4;
    private static final int DEFAULT_JOY_PAUSE = 5;
    ...
    private static void loadPreferences() {
        ...
        String val;
        try {
            val = prefs.get("keyP0Left", null);
            if (val != null) KEY_P0_LEFT = Integer.parseInt(val);
            val = prefs.get("keyP0Up", null);
            if (val != null) KEY_P0_UP = Integer.parseInt(val);
            val = prefs.get("keyP0Right", null);
            if (val != null) KEY_P0_RIGHT = Integer.parseInt(val);
            val = prefs.get("keyP0Down", null);
            if (val != null) KEY_P0_DOWN = Integer.parseInt(val);
            val = prefs.get("keyP0Button", null);
            if (val != null) KEY_P0_BUTTON = Integer.parseInt(val);
            val = prefs.get("keyP0Button2", null);
            if (val != null) KEY_P0_BUTTON2 = Integer.parseInt(val);
            val = prefs.get("keyP1Left", null);
            if (val != null) KEY_P1_LEFT = Integer.parseInt(val);
            val = prefs.get("keyP1Up", null);
            if (val != null) KEY_P1_UP = Integer.parseInt(val);
            val = prefs.get("keyP1Right", null);
            if (val != null) KEY_P1_RIGHT = Integer.parseInt(val);
            val = prefs.get("keyP1Down", null);
            if (val != null) KEY_P1_DOWN = Integer.parseInt(val);
            val = prefs.get("keyP1Button", null);
            if (val != null) KEY_P1_BUTTON = Integer.parseInt(val);
            val = prefs.get("keyP1Button2", null);
            if (val != null) KEY_P1_BUTTON2 = Integer.parseInt(val);
            val = prefs.get("joyP0Device", null);
            if (val != null) JOY_P0_DEVICE = Integer.parseInt(val);
            val = prefs.get("joyP0XAxis", null);
            if (val != null) JOY_P0_XAXIS = Integer.parseInt(val);
            val = prefs.get("joyP0XAxisSignal", null);
            if (val != null) JOY_P0_XAXIS_SIGNAL = Integer.parseInt(val);
            val = prefs.get("joyP0YAxis", null);
            if (val != null) JOY_P0_YAXIS = Integer.parseInt(val);
            val = prefs.get("joyP0YAxisSignal", null);
            if (val != null) JOY_P0_YAXIS_SIGNAL = Integer.parseInt(val);
            val = prefs.get("joyP0Button", null);
            if (val != null) JOY_P0_BUTTON = Integer.parseInt(val);
            val = prefs.get("joyP0Button2", null);
            if (val != null) JOY_P0_BUTTON2 = Integer.parseInt(val);
            val = prefs.get("joyP0Reset", null);
            if (val != null) JOY_P0_RESET = Integer.parseInt(val);
            val = prefs.get("joyP0Select", null);
            if (val != null) JOY_P0_SELECT = Integer.parseInt(val);
            val = prefs.get("joyP0Pause", null);
            if (val != null) JOY_P0_PAUSE = Integer.parseInt(val);
            val = prefs.get("joyP0FastSpeed", null);
            if (val != null) JOY_P0_FAST_SPPED = Integer.parseInt(val);
            val = prefs.get("joyP0Deadzone", null);
            if (val != null) JOY_P0_DEADZONE = Integer.parseInt(val);
            val = prefs.get("joyP0PaddleCenter", null);
            if (val != null) JOY_P0_PADDLE_CENTER = Integer.parseInt(val);
            val = prefs.get("joyP0PaddleSens", null);
            if (val != null) JOY_P0_PADDLE_SENS = Integer.parseInt(val);
            val = prefs.get("joyP1Device", null);
            if (val != null) JOY_P1_DEVICE = Integer.parseInt(val);
            val = prefs.get("joyP1XAxis", null);
            if (val != null) JOY_P1_XAXIS = Integer.parseInt(val);
            val = prefs.get("joyP1XAxisSignal", null);
            if (val != null) JOY_P1_XAXIS_SIGNAL = Integer.parseInt(val);
            val = prefs.get("joyP1YAxis", null);
            if (val != null) JOY_P1_YAXIS = Integer.parseInt(val);
            val = prefs.get("joyP1YAxisSignal", null);
            if (val != null) JOY_P1_YAXIS_SIGNAL = Integer.parseInt(val);
            val = prefs.get("joyP1Button", null);
            if (val != null) JOY_P1_BUTTON = Integer.parseInt(val);
            val = prefs.get("joyP1Button2", null);
            if (val != null) JOY_P1_BUTTON2 = Integer.parseInt(val);
            val = prefs.get("joyP1Reset", null);
            if (val != null) JOY_P1_RESET = Integer.parseInt(val);
            val = prefs.get("joyP1Select", null);
            if (val != null) JOY_P1_SELECT = Integer.parseInt(val);
            val = prefs.get("joyP1Pause", null);
            if (val != null) JOY_P1_PAUSE = Integer.parseInt(val);
            val = prefs.get("joyP1FastSpeed", null);
            if (val != null) JOY_P1_FAST_SPPED = Integer.parseInt(val);
            val = prefs.get("joyP1Deadzone", null);
            if (val != null) JOY_P1_DEADZONE = Integer.parseInt(val);
            val = prefs.get("joyP1PaddleCenter", null);
            if (val != null) JOY_P1_PADDLE_CENTER = Integer.parseInt(val);
            val = prefs.get("joyP1PaddleSens", null);
            if (val != null) JOY_P1_PADDLE_SENS = Integer.parseInt(val);
            val = prefs.get("lastROMFileChosen", null);
            if (val != null) LAST_ROM_LOAD_FILE_CHOSEN = val;
            val = prefs.get("lastROMURLChosen", null);
            if (val != null) LAST_ROM_LOAD_URL_CHOSEN = val;
            val = prefs.get("lastROMSaveFileChosen", null);
            if (val != null) LAST_ROM_SAVE_FILE_CHOSEN = val;
        } catch (Exception e) {
            // Give up
        }
    }
    ...
    private static void processProperties() {
        ...
        try {
            ...
            val = props.getProperty("CARTRIDGE_LABEL");
            if (val != null && !val.isEmpty()) CARTRIDGE_LABEL = val;
