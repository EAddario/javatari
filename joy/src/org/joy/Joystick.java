...
public class Joystick {
    ...
    private float getAxisDirection(int xAxis, int xSig, int yAxis, int ySig) {
    ...
    public int getAxisDirectionCardinal(int xAxis, int xSig, int yAxis, int ySig) {
        ...
        if ((x < 0 ? -x : x) < digitalThreshold) x = 0;
        else x *= xSig;
        if ((y < 0 ? -y : y) < digitalThreshold) y = 0;
        else y *= ySig;
        if (x == 0 && y == 0) return CENTER;
